...
public class DepositServiceHelper {
	...
	public DepositResponse processResponse(ResponseEntity<String> httpResponse) {
		...
		if (httpResponse.getStatusCode().is2xxSuccessful()) {// Success
			...
			TrustlyDepositResponse depositResponse = gson.fromJson(body, TrustlyDepositResponse.class);
			log.info("Parsed TrustlyDepositResponse: {}", depositResponse);

			if (depositResponse != null && depositResponse.getResult() != null
		...
		if (httpResponse.getStatusCode().is4xxClientError() || httpResponse.getStatusCode().is5xxServerError()) {
			...
			TrustlyErrorResponse errorResponse = gson.fromJson(httpResponse.getBody(), TrustlyErrorResponse.class);
			...
			if (errorResponse != null && errorResponse.getError() != null) {
				...
				throw new TrustlyProviderException(ErrorEnum.TRUSTLY_ERROR.getErrorCode(), trustlyErrorMessage,
		...
		throw new TrustlyProviderException(ErrorEnum.ERROR_PROCESSING_TRUSTLY_RESPONSE.getErrorCode(),
				ErrorEnum.ERROR_PROCESSING_TRUSTLY_RESPONSE.getErrorMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
	...
	public HttpRequest prepareDepositRequest(DepositRequest depositRequest) {
		...
		Attributes attributes = Attributes.builder().country(depositRequest.getCountry())
				.locale(depositRequest.getLocale()).currency(depositRequest.getCurrency())
				.amount(depositRequest.getAmount()).firstName(depositRequest.getFirstName())
				.lastName(depositRequest.getLastName()).email(depositRequest.getEmail())

				.successUrl(depositRequest.getSuccessUrl()).failUrl(depositRequest.getFailUrl()).build();

		DataPayload data = DataPayload.builder().username(merchantUsername).password(merchantPassword)
				.notificationUrl(notificationUrl).endUserId(depositRequest.getEndUserId())
				.messageId(depositRequest.getTxnReference()).attributes(attributes).build();

		// sign(String method, String uuid, JsonElement data,
		String signature = RS256SignerVerifier.sign(Constant.DEPOSIT, depositRequest.getTxnReference(),
				gson.toJson(data));
		...
		if (signature == null || signature.isEmpty()) {
			...
			throw new TrustlyProviderException(ErrorEnum.ERROR_GENERATING_SIGNATURE.getErrorCode(),
					ErrorEnum.ERROR_GENERATING_SIGNATURE.getErrorMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
		}

		Params params = Params.builder().signature(signature).uuid(depositRequest.getTxnReference()).data(data).build();

		DepositRequestWrapper request = DepositRequestWrapper.builder().method(Constant.DEPOSIT).params(params)
				.version(Constant.VERSION_1_1).build();
